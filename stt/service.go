...
// Service handles speech-to-text transcription.
type Service struct {
...
// Request contains options for transcription.
type Request struct {
...
// Response contains the transcription result.
...
// Utterance represents a speaker segment.
...
func (s *Service) Transcribe(ctx context.Context, req *Request) (*Response, error) {
	if req.FileURL == "" && len(req.FileBytes) == 0 {
		return nil, fmt.Errorf("stt: either FileURL or FileBytes must be provided")
	}
...
func (s *Service) transcribeWithURL(ctx context.Context, req *Request) (*Response, error) {
	...
	resp, err := s.apiClient.SpeechToText(ctx, body, api.SpeechToTextParams{})
	if err != nil {
		return nil, err
	}
...
func (s *Service) transcribeWithFileUpload(ctx context.Context, req *Request) (*Response, error) {
	...
	if _, err := filePart.Write(req.FileBytes); err != nil {
		return nil, fmt.Errorf("stt: write file data: %w", err)
	}
	...
	if err := writer.WriteField("model_id", modelID); err != nil {
		return nil, fmt.Errorf("stt: write model_id: %w", err)
	}

	// Add optional fields
	if req.LanguageCode != "" {
		if err := writer.WriteField("language_code", req.LanguageCode); err != nil {
			return nil, fmt.Errorf("stt: write language_code: %w", err)
		}
	}
	if req.Diarize {
		if err := writer.WriteField("diarize", "true"); err != nil {
			return nil, fmt.Errorf("stt: write diarize: %w", err)
		}
	}
	if req.NumSpeakers > 0 {
		if err := writer.WriteField("num_speakers", fmt.Sprintf("%d", req.NumSpeakers)); err != nil {
			return nil, fmt.Errorf("stt: write num_speakers: %w", err)
		}
	}
	if req.TagAudioEvents {
		if err := writer.WriteField("tag_audio_events", "true"); err != nil {
			return nil, fmt.Errorf("stt: write tag_audio_events: %w", err)
		}
	}
	...
	// Create HTTP request
	httpReq, err := http.NewRequestWithContext(ctx, "POST", s.baseURL+"/v1/speech-to-text", &buf)
	if err != nil {
	...
	if err != nil {
		return nil, fmt.Errorf("stt: execute request: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("stt: read response: %w", err)
	}

	// Check status code
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("stt: API error (status %d): %s", resp.StatusCode, string(respBody))
	}

	// Parse response
	var result sttAPIResponse
	if err := json.Unmarshal(respBody, &result); err != nil {
		return nil, fmt.Errorf("stt: parse response: %w", err)
	}
...
func (r *sttAPIResponse) toTranscriptionResponse() *Response {
	result := &Response{
		Text:         r.Text,
...
func (s *Service) parseTranscriptionResponse(resp api.SpeechToTextRes) (*Response, error) {
	switch r := resp.(type) {
	case *api.SpeechToTextOK:
		if !r.IsSpeechToTextChunkResponseModel() {
			return nil, fmt.Errorf("stt: unexpected response format")
		}
	...
	case *api.HTTPValidationError:
		if len(r.Detail) > 0 {
			return nil, fmt.Errorf("stt: validation error for %s: %s",
				formatValidationLocation(r.Detail[0].Loc), r.Detail[0].Msg)
		}
		return nil, fmt.Errorf("stt: validation error (status 422)")
	case *api.SpeechToTextAcceptedApplicationJSON:
		return nil, fmt.Errorf("stt: async transcription not supported (status 202)")
	default:
		return nil, fmt.Errorf("stt: unexpected response type")
	}
}

// TranscribeURL transcribes audio from a URL.
func (s *Service) TranscribeURL(ctx context.Context, url string) (*Response, error) {
	return s.Transcribe(ctx, &Request{FileURL: url})
}

// TranscribeWithDiarization transcribes audio with speaker identification.
func (s *Service) TranscribeWithDiarization(ctx context.Context, url string) (*Response, error) {
	return s.Transcribe(ctx, &Request{
		FileURL: url,
		Diarize: true,
	})
}
...
func (s *Service) TranscribeFile(ctx context.Context, fileBytes []byte) (*Response, error) {
	if len(fileBytes) == 0 {
		return nil, fmt.Errorf("stt: file bytes cannot be empty")
	}
