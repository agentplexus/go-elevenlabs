...
// ConversationsService handles conversation history and analytics.
...
// ConversationFeedback represents user feedback for a conversation.
...
func (s *ConversationsService) List(ctx context.Context, opts *ListConversationsOptions) (*ListConversationsResponse, error) {
...
func (s *ConversationsService) Get(ctx context.Context, conversationID string) (*ConversationDetail, error) {
	if conversationID == "" {
...
// GetAudio retrieves the audio recording for a conversation.
func (s *ConversationsService) GetAudio(ctx context.Context, conversationID string) (io.ReadCloser, error) {
	if conversationID == "" {
		return nil, errors.New("conversation ID is required")
	}
...
func (s *ConversationsService) SubmitFeedback(ctx context.Context, conversationID string, feedback string) error {
	if conversationID == "" {
		return errors.New("conversation ID is required")
	}
	...
	_, err := s.client.apiClient.PostConversationFeedbackRoute(ctx, req, api.PostConversationFeedbackRouteParams{
		ConversationID: conversationID,
	})
	return err
}

// RunAnalysis triggers analysis on a conversation.
func (s *ConversationsService) RunAnalysis(ctx context.Context, conversationID string) error {
	if conversationID == "" {
		return errors.New("conversation ID is required")
	}

	_, err := s.client.apiClient.RunConversationAnalysis(ctx, api.RunConversationAnalysisParams{
		ConversationID: conversationID,
	})
	return err
}
...
// Search performs a text search across conversation transcripts.
func (s *ConversationsService) Search(ctx context.Context, query string, opts *ListConversationsOptions) (*ListConversationsResponse, error) {
	if query == "" {
		return nil, errors.New("query is required")
	}

	// Use the smart search endpoint via raw HTTP
...
func (s *ConversationsService) doJSON(ctx context.Context, method, path string, body, result any) error {
	...
	resp, err := http.DefaultClient.Do(httpReq) //nolint:gosec // API client, URL is fixed ElevenLabs endpoint
	if err != nil {
