...
//   - Agent branches and deployments
...
// APIClient is the interface required by the agents package.
...
// Service provides access to ElevenAgents (Conversational AI) operations.
type Service struct {
	client APIClient
}
...
func (e *APIError) Error() string {
	if e.StatusCode > 0 {
		return fmt.Sprintf("agents API error (status %d): %s", e.StatusCode, e.Message)
	}
	return fmt.Sprintf("agents API error: %s", e.Message)
}
...
func (s *Service) List(ctx context.Context, opts *ListAgentsOptions) (*ListAgentsResponse, error) {
	...
	resp, err := s.client.API().GetAgentsRoute(ctx, params)
	if err != nil {
		return nil, err
	}
...
func (s *Service) Update(ctx context.Context, agentID string, req *UpdateAgentRequest) (*Agent, error) {
	if agentID == "" {
		return nil, &APIError{Message: "agent_id is required"}
	}
	if req == nil {
...
func (s *Service) Delete(ctx context.Context, agentID string) error {
	...
	_, err := s.client.API().DeleteAgentRoute(ctx, api.DeleteAgentRouteParams{
		AgentID: agentID,
	})
	return err
}
...
func (s *Service) Duplicate(ctx context.Context, agentID string, name string) (*Agent, error) {
	if agentID == "" {
		return nil, &APIError{Message: "agent_id is required"}
	}
	...
	resp, err := s.client.API().DuplicateAgentRoute(ctx, req, api.DuplicateAgentRouteParams{
		AgentID: agentID,
	})
	if err != nil {
		return nil, err
...
func (s *Service) GetLink(ctx context.Context, agentID string) (*AgentLink, error) {
	if agentID == "" {
		return nil, &APIError{Message: "agent_id is required"}
	}

	resp, err := s.client.API().GetAgentLinkRoute(ctx, api.GetAgentLinkRouteParams{
		AgentID: agentID,
	})
	if err != nil {
		return nil, err
...
func (s *Service) UploadAvatar(ctx context.Context, agentID string, avatarData io.Reader) error {
	if agentID == "" {
		return &APIError{Message: "agent_id is required"}
	}
	...
	_, err = s.client.API().PostAgentAvatarRoute(ctx, &file, api.PostAgentAvatarRouteParams{
		AgentID: agentID,
	})
	return err
}
...
func (s *Service) GetTopics(ctx context.Context, agentID string) ([]*AgentTopic, error) {
	if agentID == "" {
		return nil, &APIError{Message: "agent_id is required"}
	}

	resp, err := s.client.API().GetAgentTopicsRoute(ctx, api.GetAgentTopicsRouteParams{
		AgentID: agentID,
	})
	if err != nil {
		return nil, err
...
func (s *Service) ListBranches(ctx context.Context, agentID string) ([]*AgentBranch, error) {
	if agentID == "" {
		return nil, &APIError{Message: "agent_id is required"}
	}

	resp, err := s.client.API().GetBranchesRoute(ctx, api.GetBranchesRouteParams{
		AgentID: agentID,
	})
	if err != nil {
		return nil, err
...
func (s *Service) GetBranch(ctx context.Context, agentID, branchID string) (*AgentBranch, error) {
	if agentID == "" {
		return nil, &APIError{Message: "agent_id is required"}
	}
	if branchID == "" {
	...
	resp, err := s.client.API().GetBranchRoute(ctx, api.GetBranchRouteParams{
		AgentID:  agentID,
		BranchID: branchID,
	})
	if err != nil {
...
func (s *Service) CreateBranch(ctx context.Context, agentID string, req *CreateBranchRequest) (*AgentBranch, error) {
	if agentID == "" {
		return nil, &APIError{Message: "agent_id is required"}
	}
	if req == nil {
...
func (s *Service) UpdateBranch(ctx context.Context, agentID, branchID string, req *UpdateBranchRequest) (*AgentBranch, error) {
	if agentID == "" {
		return nil, &APIError{Message: "agent_id is required"}
	}
	if branchID == "" {
	...
	resp, err := s.client.API().UpdateBranchRoute(ctx,
...
func (s *Service) MergeBranch(ctx context.Context, agentID, sourceBranchID, targetBranchID string) error {
	if agentID == "" {
		return &APIError{Message: "agent_id is required"}
	}
	if sourceBranchID == "" {
	...
	_, err := s.client.API().MergeBranchIntoTarget(ctx,
		body,
...
func (s *Service) Deploy(ctx context.Context, agentID string, deployments []DeploymentRequest) error {
	if agentID == "" {
		return &APIError{Message: "agent_id is required"}
	}
	...
	_, err := s.client.API().CreateAgentDeploymentRoute(ctx,
...
func (s *Service) GetKnowledgeBaseSize(ctx context.Context, agentID string) (float64, error) {
	if agentID == "" {
		return 0, &APIError{Message: "agent_id is required"}
	}

	resp, err := s.client.API().GetAgentKnowledgeBaseSize(ctx, api.GetAgentKnowledgeBaseSizeParams{
		AgentID: agentID,
	})
	if err != nil {
		return 0, err
...
func (s *Service) GetWidget(ctx context.Context, agentID string) (*WidgetConfig, error) {
	if agentID == "" {
		return nil, &APIError{Message: "agent_id is required"}
	}
...
func (s *Service) doJSON(ctx context.Context, method, path string, body any, result any) error {
